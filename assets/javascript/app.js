...
db.ref("/Chatrooms").on("child_changed", function (snap, snapKey) {
  ...
  if (
    snap.val().participant1UserId == myUserId ||
    snap.val().participant2UserId == myUserId
  ) {
    if (snap.val().participant1 !== "" && snap.val().participant2 !== "") {
      ...
      let displayOtherParticipantTxt =
        "Your Five-Minute Chat buddy is " + otherParticipantName;
...
function postMyMessage(sender, timeSent, msg, gifUrl, topicWord) {
  $("#messagesBox").prepend(
    "<div class='row mt-3'><div class='col-5'></div>" +
      ...
      timeSent +
      "</small><span class='topic-word'>&#8220" +
      topicWord +
      "&#8221</span></p>" +
      "<p class='card-text'>" +
      msg +
...
function getGifSendMessage(strTopic, strMessage) {
  ...
  var queryURL =
    "https://api.giphy.com/v1/gifs/search?api_key=dSe8JxZC5c32HRcUeWDIT7n5R8PYUmTF&q=" +
    strTopic +
    "&rating=g&limit=1&offset=" +
    placeholder;
...
window.onunload = function (event) {
  ...
  let newMessageObj = {
    participantName: "Five-Minute Chat",
    message:
      otherParticipantName + " has logged off. Hit Refresh to start again!",
    messageTime: firebase.database.ServerValue.TIMESTAMP,
    gifUrl: "./assets/dog.gif",
    chatroomId: myThreadId,
    searchWord: "",
...
$("#btnStart").on("click", function (event) {
  ...
  if (myScreenName === "" && $("#nameInput").val() !== "") {
    ...
    if (myThreadId === "") {
      ...
      $("#chattersInfoLine").text(
        "Waiting for your chat buddy to log on. (or open new browser tab for demo)"
      );
    } else {
...
function chooseKeyWordAndSendMessage(ary, msg) {
  ...
  if (ary.length > 0) {
    ...
    var queryURL =
      ...
      element +
      "?key=ad5c6350-dd95-47d6-adbf-12ce92ca73ce";
